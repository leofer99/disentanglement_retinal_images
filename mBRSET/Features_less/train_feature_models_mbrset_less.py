...
def save_image(plt, img_name, model_name='DT'):
    parent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    image_dir = os.path.join(parent_dir, f'Results/mBRSET_less_features/{model_name}')
...
def save_results(df_test, y_pred, y_pred_prob, model_name):
    ...
    name=f'mBRSET_less_{model_name}_feature_model.csv'

    parent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    results_dir = os.path.join(parent_dir, f'Results/mBRSET_less_features/{model_name}')
...
# Best Parameters: {'ccp_alpha': 0.001, 'class_weight': None, 'criterion': 'gini', 'max_depth': 5, 'max_features': 0.75, 'max_leaf_nodes': None, 'min_impurity_decrease': 0.0, 'min_samples_leaf': 7, 'min_samples_split': 2, 'splitter': 'random'}
...
def apply_xg_boost(X_train, y_train, X_test, y_test, X_val, y_val, save=False):
    ...
    scale_pos_weight_= sum(y_train==0) / sum(y_train==1) #weights for class imbalancement
...
# Best Parameters: {'alpha': 0.5, 'colsample_bytree': 1.0, 'gamma': 0.1, 'lambda': 0.1, 'learning_rate': 0.05, 'max_depth': 6, 'min_child_weight': 5, 'n_estimators': 100, 'subsample': 0.7}
...
# Best Parameters: {'alpha': 0.6, 'colsample_bytree': 1.0, 'gamma': 0.05, 'lambda': 0.05, 'learning_rate': 0.07, 'max_depth': 6, 'min_child_weight': 5, 'n_estimators': 70, 'subsample': 0.7}
    ...
    # 'subsample': [0.5, 0.6, 0.7, 0.75],
    # 'colsample_bytree': [0.8, 0.9, 1.0],
    # 'gamma': [0.05, 0.1, 0.15],
    # 'lambda': [0.05, 0.1, 0.15],
    # 'alpha': [0.4, 0.5, 0.6],
    # }
...
# Best Parameters: {'C': 0.3658862876254181, 'class_weight': 'balanced', 'fit_intercept': True, 'max_iter': 50, 'penalty': 'l2', 'solver': 'newton-cg', 'tol': 0.01}

    lr_model = LogisticRegression(
        C= 0.3658862876254181, 
...
for model, model_name in zip(
    ...
    if model is not None:
        ...
        model_path=  os.path.join(save_directory, f'mbrset_less_2classicdr_{model_name}.pkl')
